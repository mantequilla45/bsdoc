...
export async function GET(req: NextRequest) {
    ...
    try {
        const { data, error } = await supabaseAdmin.rpc('get_age_bracket_counts');

        if (error) {
            console.error('Error fetching age bracket counts:', error);
            return NextResponse.json({ error: 'Failed to fetch age bracket counts' }, { status: 500 });
        }
