...
export async function GET(req: NextRequest) {
    ...
    try {
        const { data: weightData, error } = await supabaseAdmin.rpc('get_weight_bracket_counts');

        if (error) {
            console.error('Error fetching weight bracket counts:', error);
            return NextResponse.json({ error: 'Failed to fetch weight bracket counts' }, { status: 500 });
        }

        return NextResponse.json({ data: weightData }, { status: 200 });
    } catch (error: unknown) {
