...
function AdminDashboard() {
    ...
    useEffect(() => {
        ...
        if (symptomCounts.length > 0 && symptomChartRef.current) {
            chartInstances.symptom = renderBarChart(symptomChartRef.current, [...symptomCounts], 'Symptom Counts'); // Pass a copy
        } else {
            destroyChart(symptomChartRef.current);
        }

        if (healthConditionCounts.length > 0 && healthConditionChartRef.current) {
            chartInstances.healthCondition = renderBarChart(healthConditionChartRef.current, [...healthConditionCounts], 'Health Condition Counts'); // Pass a copy
        } else {
            destroyChart(healthConditionChartRef.current);
        }

        if (ageBracketCounts.length > 0 && ageBracketChartRef.current) {
            chartInstances.ageBracket = renderLineChart(ageBracketChartRef.current, [...ageBracketCounts], 'User Age Distribution'); // Pass a copy
        } else {
            destroyChart(ageBracketChartRef.current);
        }

        if (weightBracketCounts.length > 0 && weightBracketChartRef.current) {
            chartInstances.weightBracket = renderWeightLineChart(weightBracketChartRef.current, [...weightBracketCounts], 'User Weight Distribution'); // Pass a copy
        } else {
    ...
    const renderBarChart = (
        ...
        // *** ADD SORTING HERE ***
        const sortedData = data.sort((a, b) => a.count - b.count); // Sort by count ascending

        // Access properties using correct names from the interface from the SORTED data
        const labels: string[] = sortedData.map(item => formatLabel(item.status_value));
        const counts: number[] = sortedData.map(item => item.count);
    ...
    const renderLineChart = (
        ...
        // *** MODIFY SORTING HERE: Sort by age bracket order ***
        const sortedData = data.sort((a, b) => {
            // Extract the starting age number from the bracket string (e.g., '18' from '18-24')
            const startAgeA = parseInt(a.age_bracket.split('-')[0], 10) || 0;
            const startAgeB = parseInt(b.age_bracket.split('-')[0], 10) || 0;
            return startAgeA - startAgeB; // Sort numerically by starting age
        });

        const labels: string[] = sortedData.map(item => formatLabel(item.age_bracket));
        const counts: number[] = sortedData.map(item => item.count);
        ...
        const chartOptions: ChartOptions<'line'> = {
            scales: {
                ...
                // Ensure category scale type for labels
                x: { type: 'category', title: { display: true, text: 'Age Bracket' } }
            },
            plugins: {
                title: { display: false }, // Title is handled by surrounding div
                legend: { display: false }
    ...
    const renderWeightLineChart = (
        ...
        // *** MODIFY SORTING HERE: Sort by weight bracket order ***
        const sortedData = data.sort((a, b) => {
            // Extract the starting weight number from the bracket string (e.g., '40' from '40-49')
            const startWeightA = parseInt(a.weight_bracket.split('-')[0], 10) || 0;
            const startWeightB = parseInt(b.weight_bracket.split('-')[0], 10) || 0;
            return startWeightA - startWeightB; // Sort numerically by starting weight
        });

        const labels: string[] = sortedData.map(item => formatLabel(item.weight_bracket));
        const counts: number[] = sortedData.map(item => item.count);
        const chartData: ChartData<'line', number[], string> = {
            labels: labels,
            datasets: [{
        ...
        const chartOptions: ChartOptions<'line'> = {
            scales: {
                y: { beginAtZero: true, title: { display: true, text: 'Number of Users' }, ticks: { stepSize: 1, precision: 0 } },
                // Ensure category scale type for labels
                x: { type: 'category', title: { display: true, text: 'Weight Bracket (kg)' } }
            },
            plugins: {
                title: { display: false }, // Title is handled by surrounding div
                legend: { display: false }
                        ...
                        <>
                            ...
                            <div className="overflow-auto flex-grow border rounded"> {/* Scrollable table container */}
                                <table className="w-full border-collapse text-sm">
                                    ...
                                    <tbody>
                                        {/* Keep table sorting descending or change to a.count - b.count for ascending */}
                                        {symptomCounts
                                            .sort((a, b) => b.count - a.count) // Optional: Sort table separately if needed
                                            .map((item) => (
